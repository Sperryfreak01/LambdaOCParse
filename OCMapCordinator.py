...
def startParser():
    ...
    topic = sns.Topic('arn:aws:sns:us-west-2:685804734565:BotterMap')
    for city in cityList:
        logger.info('queing parser for %s' % cityList[city])
        payload = json.dumps({'default': 'Map Maker Notification','lambda': json.dumps({'city':city,'days': 7})})
        print (payload)
...
def lambda_handler(event, context):
    logger.info('Starting OC Map Cordinator')
    logger.debug(event)
    startParser()
    logger.info('Stopping OC Map Cordinator')
